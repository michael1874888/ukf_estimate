...
void box_cb(const std_msgs::Float32MultiArray::ConstPtr& msg) {
    box = *msg;
    //ROS_INFO("box");
        //ROS_INFO("x_center: %.2f y_center: %.2f", (box.data[2]*image_width + box.data[4]*image_width)/2, (box.data[3]*image_height + box.data[5]*image_height)/2);
//ROS_INFO("x:%d	y:%d	width:%d	height:%d",msg->x,msg->y,msg->width,msg->height);
}
////////////////////Global variable////////////////
double L;
double dt;
...
Eigen::MatrixXd state_to_measure(Eigen::MatrixXd sigma_state){
    ...
    for(int i=0;i<x_sigmavector_size;i++){
        ...
        predict_sigma_measure( member_mx1 ,i) =   fx*sigma_state(member_x1,i) + cx;
        predict_sigma_measure( member_mx2 ,i) =   fy*sigma_state(member_x2,i) + cy;
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        measure_vector<<center_u, center_v;
        //measure_vector<<((center_u - cx)/fx), ((center_v - cy)/fy);
        ROS_INFO("u:%.3f v:%.3f", center_u, center_v);
        ROS_INFO("mx1:%.3f mx2:%.3f", (center_u - cx)/fx, (center_v - cy)/fy);
        ...
        measure_value.z = (host_mocap.pose.position.y - 0);     //car is set to origin
