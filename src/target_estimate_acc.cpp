...
Eigen::MatrixXd dynamics(Eigen::MatrixXd sigma_state){
    ...
    for(int i=0;i<x_sigmavector_size;i++){
        ...
        Eigen::VectorXd aq;
        aq.setZero(3);
        aq << sigma_state(member_aqx,i), sigma_state(member_aqy,i), sigma_state(member_aqz,i);
        ...
        predict_sigma_state(member_aqx,i) =  aq_(0);
        predict_sigma_state(member_aqy,i) =  aq_(1);
        predict_sigma_state(member_aqz,i) =  aq_(2);
...
int main(int argc, char **argv)
{
    ...
    ros::param::get("~P_init_10", P_init(10,10));
    ros::param::get("~P_init_11", P_init(11,11));
    ...
    ros::param::get("~process_noise_10", process_noise(10,10));
    ros::param::get("~process_noise_11", process_noise(11,11));
    ...
    while(ros::ok()){
        ...
        estimate_value.feature.z = (1/x(2));
