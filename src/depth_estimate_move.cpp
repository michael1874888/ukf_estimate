...
//    ROS_INFO("vgx:%.3f vgy:%.3f vgz:%.3f",(rot_c2g*camera_vel)(0),(rot_c2g*camera_vel)(1),(rot_c2g*camera_vel)(2));
//    ROS_INFO("target vx:%.3f vy:%.3f vz:%.3f",target_g2cvel(0),target_g2cvel(1),target_g2cvel(2));
    //ROS_INFO("target wx:%.3f wy:%.3f wz:%.3f",target_g2cwvel(0),target_g2cwvel(1),target_g2cwvel(2));
    ...
    for(int i=0;i<x_sigmavector_size;i++){
        ...
        //double xq = sigma_state(member_xq,i);
        //double yq = sigma_state(member_yq,i);
        //double zq = sigma_state(member_zq,i);
        //double vqx = sigma_state(member_vqx,i);
        //double vqy = sigma_state(member_vqy,i);
        //double vqz = sigma_state(member_vqz,i);
        ...
        //xq_ = xq + (rotationB2C_z*rotationB2C_y*rotation_x*rotation_y*rotation_z).transpose*vqx*dt;
        //yq_ = yq + (rotationB2C_z*rotationB2C_y*rotation_x*rotation_y*rotation_z).transpose*vqy*dt;
        //zq_ = zq + (rotationB2C_z*rotationB2C_y*rotation_x*rotation_y*rotation_z).transpose*vqz*dt;
    ...
    //ROS_INFO("vqx_:%.3f vqy_:%.3f vqz_:%.3f",(rotationB2C_z*rotationB2C_y*rotation_x*rotation_y*rotation_z).transpose*predict_sigma_state(member_vqx,0),(rotationB2C_z*rotationB2C_y*rotation_x*rotation_y*rotation_z).transpose*predict_sigma_state(member_vqy,0),(rotationB2C_z*rotationB2C_y*rotation_x*rotation_y*rotation_z).transpose*predict_sigma_state(member_vqz,0));
...
//        ROS_INFO("carest:x:%.3f  y:%.3f  z:%.3f", host_mocap.pose.position.x+rel_posec2g(0), host_mocap.pose.position.y+rel_posec2g(1), host_mocap.pose.position.z+rel_posec2g(2));
        ...
        measure_value.z = (host_mocap.pose.position.y - car_pose.pose.position.y);
