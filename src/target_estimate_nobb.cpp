...
Eigen::MatrixXd state_to_measure(Eigen::MatrixXd sigma_state){
    ...
    for(int i=0;i<x_sigmavector_size;i++){
        ...
        predict_sigma_measure( member_mz ,i) =   sigma_state(member_x3,i);
...
//        ROS_INFO("vgx:%.3f vgy:%.3f vgz:%.3f",(rot*camera_vel)(0),(rot*camera_vel)(1),(rot*camera_vel)(2));
        ...
        bb_z = (model_width*fx/(box.data[4]*image_width - box.data[2]*image_width)+model_height*fy/(box.data[5]*image_height - box.data[3]*image_height))/2;
        measure_vector<<center_u, center_v, 1/bb_z, host_mocap.pose.position.x, host_mocap.pose.position.y, host_mocap.pose.position.z;
        //measure_vector<<((center_u - cx)/fx), ((center_v - cy)/fy);
        ROS_INFO("u:%.3f v:%.3f z:%.3f", center_u, center_v, bb_z);
        ROS_INFO("mx1:%.3f mx2:%.3f mz:%.3f", (center_u - cx)/fx, (center_v - cy)/fy, bb_z);
